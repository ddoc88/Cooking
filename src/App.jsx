...
Text: ${fullText}`
  ...
  if (page === "home") {
    return (
      <div style={{ padding: "20px", fontFamily: "sans-serif", background: theme==="dark"?"#111":"#f9f9f9", minHeight: "100vh" }}>
        <h1>Cooking App</h1>
        <button onClick={() => setPage("recipes")} style={{ display:"block", margin:"10px 0", width:"100%" }}>Recipes</button>
        <button onClick={() => setPage("add")} style={{ display:"block", margin:"10px 0", width:"100%" }}>+ Add Recipe</button>
        <button onClick={toggleTheme} style={{ display:"block", margin:"10px 0", width:"100%" }}>Toggle Theme</button>
      </div>
    );
  }
  ...
  if (page === "add") {
    return (
      <div style={{ padding: "20px" }}>
        <h1>Add Recipe</h1>
        <input type="text" placeholder="Recipe Name" id="recipeName" style={{ width: "100%", marginBottom: "10px" }} />
        <input type="file" onChange={(e) => handlePDFUpload(e.target.files[0])} />
        <button style={{ display: "block", marginTop: "10px" }} onClick={() => setPage("home")}>Back</button>
      </div>
    );
  }
      ...
      <div style={{ padding: "20px" }}>
        ...
        {recipes.map((r, i) => (
          <div key={i} style={{ border: "1px solid #ccc", margin: "10px 0", borderRadius:"10px", padding:"10px" }}>
            ...
            <button style={{ background:"orange", color:"white", marginTop:"5px", width:"49%" }} onClick={() => editRecipe(i,r)}>Edit</button>
            <button style={{ background:"red", color:"white", marginTop:"5px", width:"49%" }} onClick={() => deleteRecipe(i)}>Delete</button>
          </div>
        ))}
        <button onClick={() => setPage("add")} style={{ display:"block", margin:"10px 0", width:"100%" }}>+ Add Recipe</button>
      </div>
